...
PATH_TO_FINAL_FILE = os.path.join(PATH_TO_FINAL_FOLDER, "unitedStates.csv")
...
def extract_debate_text(content_div):
    ...
    for paragraph in content_div.find_all("p"):
        ...
        if name_pattern.match(text):
            speaker = name_pattern.match(text).group(0)

            # Skip non-speech blocks
            # if speaker in {'PARTICIPANTS:', 'MODERATORS:'}:
            #     continue

            # Save current block and start new one
            if current_text:
                processed_texts.append({'text': current_text, "speaker": speaker})
