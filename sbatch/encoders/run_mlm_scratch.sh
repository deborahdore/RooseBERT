...
export WANDB_PROJECT="Masked_Language_Modelling_SCRATCH"
...
RUN_NAME="${MODEL_NAME}-batch$((BATCH * N_GPUS * GRAD_ACC))-lr${LR}-SCRATCH"
...
python -m torch.distributed.launch --nproc_per_node=${N_GPUS} \
        ...
        src/run_mlm.py \
        --model_type "bert" \
        --tokenizer_name "./Tokenizers/tokenizer_$TYPE" \
        --lowercase $LOWERCASE \
