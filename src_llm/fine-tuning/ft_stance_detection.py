...
LABEL2ID_MAP = {'con': 0, 'pro': 1, 'neutral': 2}
ID2LABEL_MAP = {0: 'con', 1: 'pro', 2: 'neutral'}
...
def load_dataset(base_path):
    ...
    for file in ["train.csv", "dev.csv", "test.csv"]:
        ...
        df['label'] = df['label'].map(ID2LABEL_MAP)
...
if __name__ == '__main__':
    ...
    dataset_dict = load_dataset("./data/stance_detection/")
    ...
    now = datetime.now()
    trainer_filepath = f"./logs/{model_id}/stance_detection/{now.strftime('%d/%m/%y:%H:%M')}"
    ...
    results_df.to_csv(f"./logs/{model_id}/results_stance_detection.csv", index=False)
